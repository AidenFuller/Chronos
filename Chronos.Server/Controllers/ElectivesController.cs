...
namespace Chronos.Server.Controllers
{
    ...
    public class ElectivesController : ControllerBase
    {
        ...
        public IEnumerable<Course> Get(int DegreeID, int MajorID)
        {
            ...
            //Select all courseID where it is NOT a core course for the given Degree ID
            IEnumerable<int> NotCoreCourseIDs = db.CoreCourses.Where(i => i.DegreeID != DegreeID).Select(i => i.CourseID);
            //Select all courses where it is NOT a  major core course for the given Major ID
            IEnumerable<int> NotMajorCourseIDs = db.MajorCourses.Where(i => i.MajorID != MajorID).Select(i => i.CourseID);
